...
public class FileServer {
    ...
    public void writeUploadFile(final HttpServerRequest request, final Long maxSize, final Handler<JsonObject> handler) {
        ...
        mimeTypeObj.put("text/plain", "txt").put("text/csv", "csv").put("application/pdf", "pdf").put("video/mp4", "mp4").put("application/zip", "zip").put("application/x-7z-compressed", "7z")
        		   .put("application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", "xlsx").put("application/vnd.openxmlformats-officedocument.wordprocessingml.document", "docx");
        ...
        request.uploadHandler(new Handler<HttpServerFileUpload>() {
            ...
            public void handle(final HttpServerFileUpload upload) {
        		...
        		metadata.put("name", upload.name()).put("filename", upload.filename()).put("content-type", upload.contentType());
        		metadata.put("content-transfer-encoding", upload.contentTransferEncoding()).put("charset", upload.charset()).put("size", upload.size());
        		...
        		if( ! mimeTypeObj.containsKey(content_type) ) {
                    ...
                    final String uploadedPath = basePath + UUID.randomUUID().toString() +"." + mimeTypeObj.getString(content_type);
            ...
            private void doUpload(final HttpServerFileUpload upload, final JsonObject metadata) {
                upload.endHandler(new Handler<Void>() {
                    ...
                    public void handle(Void event) {
                    	if(null != metadata.getString("status") && metadata.getString("status").equals("exception")) {
                            ...
                            if (maxSize != null && maxSize < metadata.getLong("size", 0l)) {
                                    fs.delete(metadata.getString("uploadedPath"), new Handler<AsyncResult<Void>>() {
                                        @Override
                                        public void handle(AsyncResult<Void> event) {
                                        	log.info("file size [ "+ metadata.getLong("size", 0l) + " ] exceeded to maxSize [ " + maxSize + " ]. Hence deleted");
                                            if (event.failed()) {
                                                log.error(event.cause().getMessage(), event.cause());
                                            }
                                        }
                                ...
                                handler.handle(res.put("status", "error").put("message", "file.too.large"));
                            		...
                            		getLastModifiedDate(metadata.getString("uploadedPath")).onComplete(resultHandler ->{
                            		metadata.put( "lastModifiedDate", resultHandler.succeeded() ? resultHandler.result() : null); 
                            		handler.handle(res.put("uploadedPath", metadata.getString("uploadedPath")).put("status", "ok").put("message", "file uploaded").put("metadata", metadata));
                            		});
                            }
                        }
                    }

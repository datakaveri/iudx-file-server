...
public class GeoQueryFiltersDecorator implements ElasticsearchQueryDecorator {
    ...
    public Map<FilterType, List<Query>> add() {
        ...
        String shapeRelation =
                NEAR.equalsIgnoreCase(requestQuery.getString(GEO_REL)) ? WITHIN : requestQuery.getString(GEO_REL);
        ...
        LOGGER.debug("Query map at 41=="+ queryFilters);
        LOGGER.debug("Query list at 42=="+ queryList);
        LOGGER.info("Geo Query at 43 = "+geoWrapperQuery);

        queryList.add(geoWrapperQuery);
        LOGGER.debug("Querylist at 44 = " + queryList);
        LOGGER.info("QUERY FILTER == "+ queryFilters);
        return queryFilters;
    }
